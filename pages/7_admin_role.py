...
page.switched("admin_roles")

    
tab1, tab2 = st.tabs([" 📋Roles List ", "  ➕New Role "])

df_params = load_params_data()
df_role = df_params[df_params["TYPE"] == "Role"]

with tab1:
    first_container = st.container(border=True)
    with first_container:
        df = main_interaction(df_role)
...
with tab2:
    st.header(st.session_state.transl["create_role"])
    container = st.container(border=True)
    prefilled_order = int(df_role["ORDER"].max()) + 1
    with container:
        with st.form(
            ...
            if submitted:
                if all_filled:
                    admin_new_type(df_role,"Role", short_desc, long_des, order)
                else:
